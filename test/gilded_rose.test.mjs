...
describe("Gilded Rose", () => {
  ...
  test("name: foo; quality>50", () => {
    ...
    const items = gildedRose.updateQuality();
    let res = JSON.stringify(items[0]);
    expect(res).to.equal(`{"name":"Sulfuras, Hand of Ragnaros","sellIn":0,"quality":10}`);
  });

  test("name: Sulfuras, Hand of Ragnaros; sellIn=2", () => {
  ...
  test("name: Aged Brie; sellIn=0; quality=1", () => {
    ...
    const items = gildedRose.updateQuality();
    let res = JSON.stringify(items[0]);
    expect(res).to.equal(`{"name":"Aged Brie","sellIn":19,"quality":11}`);
  });
  ...
  test("name: Backstage passes to a TAFKAL80ETC concert; quality<10", () => {
    ...
    const items = gildedRose.updateQuality();
    let res = JSON.stringify(items[0]);
    expect(res).to.equal(`{"name":"Backstage passes to a TAFKAL80ETC concert","sellIn":9,"quality":4}`);
  });
  ...
  test("name=Sulfuras, Hand of Ragnaros; sellIn=5; quality=80", () => {
